...
| 0.86.0-namespace.1 | v0.86.0 | v0.86.0-namespace.1 | first release carrying the fork update-banner fix (forkFixCommit: isNewerPackageVersion compares core versions, so an upstream release equal to the fork's base is not reported as newer); L1 conflicted in agent-session.ts (upstream steer/followUp refactor into _queueUserInput) and L2 agent-repaired it by moving the namespace expansion hook into the shared chain; pipeline hydrate made best-effort after models.dev dropped kimi-coding (gates do not need the data); publish succeeded but the run's registry verification raced npm read-replica propagation (ETARGET past 8x30s) — verified manually (npmjs install + `pi --version` -> 0.86.0-namespace.1), verification window widened to 20x30s |
| 0.86.1-namespace.1 | v0.86.1 | v0.86.1-namespace.1 | same-day follow-up re-base; L1 conflicted again and L2 additionally hardened the pipeline: npm-check/suites import the gitignored generated provider catalog, so mechanical-rebase hydrates best-effort before gates and verify.sh guards with hydrate-if-missing (upstream v0.86.1 made the check chain catalog-dependent); hydrate itself is green again on this base |
| 0.87.0-namespace.1 | v0.87.0 | v0.87.0-namespace.1 | L1 conflicted in prompt-templates.ts, resource-loader.ts, and prompt-templates.test.ts: upstream v0.87.0 changed `loadTemplatesFromDir`/`loadTemplateFromFile` to return `{ templates, diagnostics }`, taught `dedupePrompts` to return diagnostics, and added the #9354 frontmatter-diagnostics test. L2 re-applied the namespace composition inside the new `addResult` helper, merged the upstream diagnostics test alongside the namespace suite, and adapted the namespace tests to the new return shape. Verification: 329 passed (+1 prompt-templates, +1 resource-loader vs 0.86.1) |
| 0.87.1-namespace.1 | v0.87.1 | v0.87.1-namespace.1 | upstream v0.87.1 refreshed docs (#9898), rewriting skills.md, packages.md, and prompt-templates.md away from the v0.87.0 layout; L1 conflicted only in those three docs (all code + tests applied cleanly). L2 took the refreshed upstream docs and re-inserted the namespace sections semantically (packages.md `### Namespace`, prompt-templates.md / skills.md `## Package Namespaces`), dropping the obsolete gallery-metadata heading in favor of upstream's inline sentence. Verification: 329 passed, unchanged vs 0.87.0; all four gates green |
